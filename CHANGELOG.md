...
- Cluster authority startup now derives a stable role-specific seed instead of reusing `node.seed`, preventing authority and deterministic discovery-query clients from sharing a PeerID and failing every authority query with `dial to self attempted`.
