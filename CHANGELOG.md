...
- Integration coverage for the grant-service opaque-relay boundary (`TestGrantServiceOpaqueRelayBoundary`, `TestGrantServiceOpaqueRelayRejectsTamperedSignature`): exercises the real `buildGrantServiceDiscoveryArtifacts` builder through direct sync to an opaque relay and consumer-side local validation, surfacing the record as a trusted `grant-service`; a tampered signature is dropped by the consumer and never surfaces as trusted. Closes #357.
...
- Opaque Discovery V3 relay forwarding now enforces per-PeerID and global record/byte quotas, one-second same-key refresh throttling, deterministic fair ordering, and a 1 MiB encoded `list_services` budget. Truncated responses set `truncated: true`; relay `/statsz` exposes aggregate opaque-cache rejection and truncation counters.
...
### Fixed
- Long-running HTTP/WebSocket `connect` processes now rediscover and rebind their pinned service endpoint once when stream creation fails with a transient reachability error, allowing expired in-memory peerstore addresses to recover through current relay announcements without replaying application requests. Peer liveness failures trigger the same serialized, cooldown-bound recovery after threshold, and failed pings now advance scheduling time instead of causing a zero-delay busy-loop.
- `ConfigRepository.Update` is now strictly fail-closed: lock acquisition failures and persistence errors are returned to the caller instead of being silently tolerated as "read-only". `EACCES`/`EPERM`/`os.ErrPermission`/`permission denied` are no longer treated as a read-only filesystem. A genuine `EROFS` is recognized by the new `config.IsReadOnlyFilesystem` helper, and only the explicit attach callers that must keep running on a read-only config volume (containerized smoke mounts) opt into an in-memory fallback with an observable warning. No mutation runs without the repository lock, and no persistence problem can produce a fake success. The `seedDiscoveredGrantServicePeer` EROFS fallback now actually applies the discovered peer (and fills missing `MembershipGrant.GrantServiceProtocol`/`GrantServicePeers`) to a deep copy of the runtime config, and non-EROFS persistence errors propagate to callers instead of being hidden. The `EnsureAttachServiceIdentity` read-only fast path no longer calls `ensureServiceState` (which can create owner keys, generate seeds, and remove stale lease/claim files): a pure `existingAttachServiceIdentityComplete` check verifies the service is already fully materialized and coherent, otherwise the transactional `ConfigRepository.Update` path runs under lock. Refs #369, #393.
- `/p2p-tunnel/1.1` now rejects oversized outer frames, nested strings/byte fields, header/value counts, capability counts, and body chunks before allocation; senders use the same frame limits, malformed trailing payload is rejected, and bounded fuzz/regression coverage protects the pre-authorization parser. Refs #365.
- Service data-plane handlers now enforce configurable pre-authentication Hello/connect-proof/header deadlines, bound local upstream dial/TLS/response-header setup, cancel upstream HTTP requests when handlers exit, and clear handshake deadlines before established HTTP/WebSocket/raw-TCP traffic so long-lived tunnels remain supported. Refs #372.
- Raw TCP connect startup now allocates its listener and adjacent admin endpoint as a rollback-safe pair, retries ephemeral-port collisions, and publishes readiness only after both binds succeed; failed admin binds no longer leave an unserved listener that accepts connections and hangs. The half-close integration test now surfaces startup errors instead of discarding them. Closes #364.
- Workspace cluster/namespace creation, scope selection, service removal, discovery-secret rotation, and expired-secret cleanup now reload and mutate config under repository locks. Concurrent resource updates are preserved, same-name cluster creation cannot mismatch authority key artifacts, and service cleanup cannot delete artifacts owned by a concurrently recreated definition.
- Attach service-identity, resolved-definition, and discovered grant-service updates plus cluster-authority discovery-peer refresh now reload and mutate config under repository locks, preventing stale snapshots from deleting service or authority endpoint updates.
- Pipe/connect lifecycle config writers now use repository transactions for definition create/update/remove and start/restart persistence. Detached readiness rollback conditionally removes only the definition written by the failed attempt, preserving concurrent replacements and unrelated config updates; pipe start/restart use saved scope instead of current selection and fail closed if post-readiness persistence fails.
- Config writes now use `0600` same-directory temp files, fsync, atomic rename, and OS advisory locking. New `ConfigRepository.Update` transactions reload under lock, preserve unknown YAML mapping fields, recover stale temp/lock files after process interruption, and prevent disjoint concurrent updates from silently overwriting each other.
- Grant requests, decisions, expiry, revocation epochs, and one-time share redemption now use path-scoped in-process plus OS advisory locks around complete JSON transactions. Pending-limit checks and insertion are atomic, files remain `0600` and atomically replaced, and concurrent authority/CLI processes no longer lose updates or exceed configured pending quotas.
- Bridge connect lease refresh no longer uses a single shared `connectRefreshResultUseful` predicate for both access and refresh leases. Access-lease usefulness is now strict (a refresh must extend the access lease by at least the minimum extension; there is no "previous broken" carve-out), while the shorter-than-previous carve-out is scoped to refresh leases in the rollover path only. `rolloverConnectLeaseLocked` now also validates the new access lease against the current one before applying artifacts, so a rollover that returns a noticeably shorter access lease is deferred instead of replacing a still-valid lease. This closes the gap where a lease with substantial remaining validity could be replaced by one that merely exceeded `now + minExtension`. Closes #356.
- `docs/runbooks/member-publish-grants.md` now correctly documents that the connect refresh lease is capped by config and membership expiry only (not by publish lease expiry), matching the server behavior in `internal/grants/server.go`; the access lease remains capped by publish lease expiry.
- `tubo config print` now masks libp2p identity seeds (`node.seed`, `bridge.service_seed`, and per-service `service_seed`) in addition to `private_key_b64`; seeds deterministically derive the host Ed25519 private key and are equivalent to private keys. `tubo doctor` now warns when a configured seed is a known demo default (`public-relay-seed`, `service-demo-seed`, `bridge-demo-seed`, `edge-seed`) or derivable from public cluster identifiers (`discovery-query-*`, `grants-*`). `.gitignore` now excludes compiled release binaries (`tubo-darwin`, `tubo-linux`, `tubo-*`) and local network signing material (`*-network-key`, `*-network-key.pub`). This is the non-breaking hardening subset of #355; the breaking identity-rotation + public-bundle re-sign, plus an explicit `--show-seed`/non-default path for `tubo create service/...` seed output (currently relied on by `tests/smoke-compose-tubo-workflow.sh`), remain staged as a follow-up. Refs #355.
- Private-cluster join now persists a stable `node.seed` when local config does not already have one, so later `tubo connect service/...` runs reuse same libp2p peer identity across restarts and candidate retries. This fixes peer-bound `namespace_members` authorization failures where join-time membership proof or later capability issuance was bound to one peer id but connect used fresh ephemeral peer ids. `tubo connect` and `tubo doctor` now also warn when a peer-bound membership capability exists but `node.seed` is missing.
- Detached TCP `connect` no longer keeps whole runtime globally degraded after per-connection client close/reset errors (`broken pipe`, `connection reset by peer`, EOF-like closes). Runtime health now recovers on successful peer/tunnel activity, transient refresh/rollover failures stay visible under authorization/backoff state instead of poisoning data-plane status forever, and successful lease refresh/rollover clears stale refresh error state.
...
- Delegated connect lease refresh now handles issuer/endpoint mismatch correctly: when a delegated refresh lease (containing `DelegationPublishLease`) is sent to a cluster grant server that rejects it with `invalid connect lease signature`, the client detects this and performs member rollover via `connect_lease.request` to obtain fresh authority-signed leases. If the service endpoint is in `ConnectGrantPeers`, direct delegated refresh succeeds without rollover. If neither path is available (no service endpoint, no rollover capability), the client fails with a clear actionable error. Logs now show the selected action path: `action=delegated_refresh`, `action=member_rollover`, or `action=no_compatible_refresh_path`.
- Grant server and service endpoint now log connect refresh requests with issuer class (`delegated` or `authority`), accept/deny status, reason for denied requests, and new access lease expiry for accepted requests.
...
- Cluster authority startup now derives a stable role-specific seed instead of reusing `node.seed`, preventing authority and deterministic discovery-query clients from sharing a PeerID and failing every authority query with `dial to self attempted`; runtime peer persistence also removes endpoints for superseded authority identities while preserving alternate paths for current PeerID.
- Grant-service discovery now builds a signed `AnnouncementV3` envelope before both pubsub publication and direct relay synchronization, so opaque relays no longer reject grant-service records with `announcement missing envelope fields`.
...
- Attach publish-authorization renewal now dedupes retries across transient grant-client peers, persists/preserves the running service identity when reloading config, mints local-authority leases for the live publisher peer, records local approvals for cluster-authority connect rollover, lets `tubo share service/...` recover from the active local approval when the service definition is missing, and surfaces exact publish/membership validation errors instead of generic unverifiable status.
...
- Detached process state updates now write via atomic rename and tolerate trailing garbage when re-reading a previously written JSON snapshot, preventing corrupted registrations from producing blank `tubo ps --all` rows when the process can still be recovered.
