...
- `stop` now accepts degraded live processes as stoppable, and raw TCP detached `connect` no longer publishes a bogus HTTP health URL on the local tunnel port.
