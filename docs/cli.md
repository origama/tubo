...
- prima di avviare il runtime, `attach` risolve l'autorizzazione di pubblicazione: usa una `PublishLease` valida esistente, la firma localmente se il nodo possiede `authority_private_key_file`, oppure invia/polla una Publish Grant request se il servizio ha `grant_service_peer`; il fallback a `ServiceClaim` resta solo compatibilità locale. Quando disponibile stampa anche un `service_share_token` connect-only copiabile per Bob (`tubo connect --token ...`). Se il token non puo' essere generato e il config ha `grant_request_id` + `grant_service_peer`, `attach` mostra un hint concreto per rifare il poll (`tubo grants request service/... --poll --peer ...`); altrimenti suggerisce il comando `tubo share service/...` da eseguire su un nodo authority.
