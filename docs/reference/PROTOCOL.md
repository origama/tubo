...
This document covers the data-plane stream protocol. Collaborative ambient service discovery is a separate control-plane concern and now uses secret-backed Discovery V3 namespace topics, with no Discovery V2 fallback for discovery-enabled namespace runtime in `v0.8.0`.
