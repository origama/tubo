...
`ps` / `get processes` refer to local processes registered on this machine. By default they now show a compact operational summary; `--wide` restores the technical table with `SERVICE KIND`, `SERVICE ID`, `SCOPE`, `PID`, `LOCAL`, and `TARGET`. Degraded/stale rows also print a short `describe`/`logs` hint instead of dumping a full diagnostic card. `stop` accepts live degraded processes too. `rm --stale` ignores live degraded processes and removes a stale process once even if older and newer on-disk aliases exist for the same connect runtime.
