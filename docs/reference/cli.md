...
- `get secrets` lists local namespace-discovery current/previous entries as metadata only.
- `describe secret/namespace-discovery/<cluster>/<namespace>` shows metadata only for the current/previous entries in that scope, including missing-file diagnostics when relevant.
