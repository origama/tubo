...
`describe process/...` and `inspect process/... --json` expose the process runtime state for local registrations. `attach` rows now include the service kind and resolved service/peer identity; when the attached service has a service-scoped grant endpoint, `describe` also shows `Grant endpoint: enabled`, the effective connect policy, and the grant protocol (`/tubo/grants/1.0`). `connect` rows also expose the selected peer/address/path alongside degraded runtime state and remaining lease lifetime, so you can tell whether a tunnel is direct or relayed.
