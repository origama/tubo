...
| C.42 | Signed public onboarding + CLI UX simplification (PR #68 follow-up) | ⏳ | In progress on prerelease branch `prerelease/pr68-signed-public-onboarding`; GitHub issues opened: #69 #71 #72 #73 #74; implemented #69 + core #71 + #72 (`tubo join` bundle mode + published signed `tubo-public.bundle`/catalog/public key under `docs/.well-known/tubo/`) + #73 (implicit public join for `attach`/`connect`/`gateway`, with `TUBO_DEFAULT_PUBLIC_BUNDLE_URL` override for prerelease/dev testing) and partial #74 (legacy role commands rejected, `attach <name> --port <port>` shorthand, CLI/site/README docs updated); canonical public host corrected to `www.tubo.click` because apex `tubo.click` does not resolve; real Linode tests found and fixed the publication path: relay entry must be a full bootstrap peer and now points to the real live public relay `relay.tubo.click:4001` with peer ID `12D3KooWFAEdvKQVbtqdo435wBxoCJxXSUpjC77MEwjVHmZk31t1`; relay startup now also auto-joins the public bundle from zero, so relay/service/client share the same swarm key by default; discovery follow-up: default `get/connect/watch` timeout raised to 20s because the old 5s live observer could miss the first 15s heartbeat; fixed live Linode discovery failure by making `get/describe/inspect/watch` auto-join the public bundle and by giving `attach` a unique generated runtime seed instead of the shared demo PeerID; fixed `connect` direct-dial UX so loopback/unspecified direct addrs are not attempted from remote machines; added WebSocket upgrade tunneling for `connect`/`attach`; follow-up polish: `connect` bridge now enables AutoRelay/hole punching, `attach` defaults to `/ip4/0.0.0.0/tcp/0` with generated PeerID seed, command help/site docs were aligned to the current UX, and installer scripts are POSIX `sh` compatible for dash; validated end-to-end on real Linode hosts with current prerelease binary: public relay `relay.tubo.click` matched the bundle swarm key, service host `172.236.202.99` attached `piweb`, client host `45.79.168.161` discovered it, `curl 127.0.0.1:9888` returned the `pi-webui` HTML, and a raw `/ws` handshake returned `101 Switching Protocols` through the relayed tunnel; previous validation also covered POST `/v1/dummy?from=linode` |
