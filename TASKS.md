...
| C.83 | Issue #141 — formalize effective scope and public-default policy helpers | ⏳ | In progress on `0.7.0.b0`: add shared scope/public-default helpers in `internal/config`, persist the minimum overlay metadata needed to distinguish the signed public overlay from manual/private overlays, and switch workspace scope resolution to the shared helper so follow-up #140 work does not re-derive policy ad hoc. |
