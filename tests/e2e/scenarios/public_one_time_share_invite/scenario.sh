...
BOB_PORT="18888"
CAROL_PORT="18889"
ATTACH_HEALTH="http://127.0.0.1:8091/healthz"
...
exec_actor_bg alice sh -lc "cd /work && exec tubo attach http://127.0.0.1:${DUMMY_PORT} --name ${SERVICE_NAME} --config /work/config.yaml --heartbeat-interval 1s > /work/logs/alice-attach-1.out 2>&1"
wait_http_ok_in_actor alice "$ATTACH_HEALTH" 90 || fail "alice attach runtime did not become healthy on first start"
...
[[ -n "$alice_services" ]] || fail "alice did not see published service on first start"

exec_actor alice sh -lc "pid=\$(ps -eo pid,args | awk '/[t]ubo attach http:\/\/127.0.0.1:${DUMMY_PORT} --name ${SERVICE_NAME}/{print \$1; exit}'); [ -n \"\$pid\" ] || exit 1; kill \"\$pid\""
sleep 2
if exec_actor alice sh -lc "curl -fsS $ATTACH_HEALTH" >/dev/null 2>&1; then
  fail "alice attach runtime still healthy after stop"
fi

exec_actor_bg alice sh -lc "cd /work && exec tubo attach http://127.0.0.1:${DUMMY_PORT} --name ${SERVICE_NAME} --config /work/config.yaml --heartbeat-interval 1s > /work/logs/alice-attach-2.out 2>&1"
wait_http_ok_in_actor alice "$ATTACH_HEALTH" 90 || fail "alice attach runtime did not become healthy after restart"
...
[[ -n "$alice_services" ]] || fail "alice did not see published service after restart"
...
response=""
for i in $(seq 1 60); do
  if response="$(exec_actor bob sh -lc "curl -fsS http://127.0.0.1:${BOB_PORT}/v1/dummy?from=public-one-time-share")"; then
    break
  fi
  sleep 1
done
...
assert_contains "$response" '"raw_query":"from=public-one-time-share"' 'response missing query marker'

exec_actor bob sh -lc "pkill -f 'tubo connect --token'" || true
rm -rf "$(actor_home bob)/config" "$(actor_home bob)/data" "$(actor_home bob)/cache" "$(actor_home bob)/clusters"
mkdir -p "$(actor_home bob)/config/tubo" "$(actor_home bob)/clusters"
cp "$(actor_home alice)/config.yaml" "$(actor_home bob)/config/tubo/config.yaml"
cp -a "$(actor_home alice)/clusters/home" "$(actor_home bob)/clusters/"

if exec_actor bob sh -lc "cd /work && tubo connect --token '$share_token' --local 127.0.0.1:${CAROL_PORT}" > "$E2E_ARTIFACTS_DIR/carol-connect.out" 2>&1; then
  fail "fresh second client unexpectedly connected with a redeemed invite"
fi
assert_file_contains "$E2E_ARTIFACTS_DIR/carol-connect.out" 'already redeemed' 'second client denial missing already-redeemed reason'
...
echo "[e2e] PASS: attach restart keeps one-time share invites one-time"
