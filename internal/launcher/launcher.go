...
func Run(ctx context.Context, deps Deps, role, configPath string, cfg cfgpkg.Config) error {
	...
	case "service":
		...
		runner, err := deps.NewService(ctx, service.Config{
			...
			BootstrapRetryInterval:        5 * time.Second,
			StreamHandshakeTimeout:        cfg.Service.HandshakeTimeout.Duration(),
			UpstreamDialTimeout:           cfg.Service.UpstreamDialTimeout.Duration(),
			UpstreamTLSHandshakeTimeout:   cfg.Service.UpstreamTLSHandshakeTimeout.Duration(),
			UpstreamResponseHeaderTimeout: cfg.Service.UpstreamResponseHeaderTimeout.Duration(),
			UpstreamIdleConnTimeout:       cfg.Service.UpstreamIdleConnTimeout.Duration(),
